...
> **Windows note:** the curl examples in this guide use bash syntax
> (`\` line continuations), and in PowerShell `curl` is an alias for
> a different command entirely. Run them in **Git Bash or WSL**, or
> translate to PowerShell like this:
>
> ```powershell
> Invoke-RestMethod -Method Post -Uri http://localhost:3777/api/run `
>   -ContentType "application/json" `
>   -Body '{"agent":"doc-processor","prompt":"Summarize README.md"}'
> ```
...
to a subset of the 10 available tools.
...
| `sandboxed_write_file` | Like `write_file`, but only accepts relative paths under `inbox/` or `reports/` — rejects absolute paths, `..`, and symlink escapes at the tool layer | `path`, `content` |
...
If it shows as running but curl still fails, remember the port is
bound to `127.0.0.1` — it is reachable only from the same machine,
and only as `localhost`/`127.0.0.1` (not the machine's LAN IP; that
is deliberate, see SETUP.txt → SECURITY). If another program already
holds 3777, put `AGENT_API_PORT=3778` in a `.env` file next to
docker-compose.yml and use that port instead.
...
| `telegram-assistant` | gemma4:e4b-chat | 4     | 8     | No     | Sandboxed messaging (bridge)   |

### All Tools

| Tool                   | Category   | Used By                                        |
|------------------------|------------|------------------------------------------------|
| `read_file`            | Filesystem | doc-processor, job-scanner, coder, telegram-assistant |
| `write_file`           | Filesystem | doc-processor, job-scanner, coder              |
| `sandboxed_write_file` | Filesystem | telegram-assistant                             |
| `list_directory`       | Filesystem | doc-processor, job-scanner, coder, telegram-assistant |
| `search_files`         | Filesystem | doc-processor, coder, telegram-assistant       |
