...
```
docker compose --profile telegram kill telegram-bridge
```

(From outside the project folder: `docker ps --filter name=telegram-bridge`
to find the container, then `docker kill <name>`.)
...
2. Then `docker compose --profile telegram kill telegram-bridge`.
...
```
docker compose --profile telegram logs --tail 5 telegram-bridge 2>&1 | tail -1
```
...
- Container output: `docker compose --profile telegram logs telegram-bridge`
